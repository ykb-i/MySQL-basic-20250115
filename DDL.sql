...
CREATE TABLE data_type(
  ...
  boolean_c BOOLEAN,
  -- VARCHAR(문자열길이): 가변길이 문자열, 문자열길이는 최대길이임
  -- 한글은 2~3바이트이기 때문에 한글자당 3바이트씩 설정하는게 좋다
  -- 5글자면 15로 설정
  varchar_c VARCHAR(5),
  -- TEXT: 고정된 크기를 가지는 문자열 (64KB)
  text_c TEXT,	

  -- VARCHAR는 모든 제약조건에서 자유로움, TEXT는 index에서 default 사용이 불가능
...
-- 테이블 컬럼 전체 수정(example_column1의 이름과 타입을 변경)
